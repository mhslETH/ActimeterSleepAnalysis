...
% Description: Cole Sleep Scoring Algorithm 
%              Preprocessing: Bandpass Filter (1.0-4.0Hz) and thresholding.
%              Activity Counts derived by zero-crossings per epoch.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Cole Sleep Scoring Algorithm
%
...
label = 70;                     % epoch interval for labeling the x-axes
hp = 1.0;                       % [Hz] cut-off frequency highpass filter
lp = 4.0;                       % [Hz] cut-off frequency lowpass filter

Scoring_Threshold = 0.1;        % 0.1 for wrist
Wake_Threshold = 0.23;          % 0.23 for wrist
Rescoring_Threshold = 0.6;      % 0.8
Nonwear_Threshold = 0.03;       % 0.01
...
count_zc = zeros(1,epochs);

for i = 1:epochs
    for j = (sum(sampling_frequencies(1:(i-1)*epoch_length)))+1 : sum(sampling_frequencies(1:i*epoch_length))
        if xyz_thr(j) > 0 && xyz_thr(j+1) < 0.01
            count_zc(1,i) = count_zc(1,i) + 1;
        elseif xyz_thr(j) < 0 && xyz_thr(j+1) > -0.01
            count_zc(1,i) = count_zc(1,i)+1;
        end
    end
end

%% Scoring Algorithm Cole

activity_count = count_zc;
j = size(activity_count);
for i = 5:(j(2)-2)
    D(i) = 0.0033*(1.06*activity_count(i-4) + 0.54*activity_count(i-3) + 0.58*activity_count(i-2) + 0.76 *activity_count(i-1) + 2.3*activity_count(i) + 0.74*activity_count(i+1) + 0.67*activity_count(i+2));
end
j = size(D);
for i = 1:j(2)
...
for i = 1:epochs - 6 - 15*(60/epoch_length)
    if status == 0
        if  nnz(activity_count(i:i+15*(60/epoch_length))) <= 1 && D(i) <= Wake_Threshold
...
%% Rescoring criterion to detect phases of low activity which are not sleep
...
axis normal
xlabel('Time')
title('Sleep/Wake with Cole Scoring Algorithm')
ylim([-0.1 1.1])
hold on
plot(activity_count_visualization/150);
hold off

clearvars sleep_period_min sleep_period sleep_start sleep_start_pos  wake_up wake_up_pos sleep_overwiev wake_time ...
    sleep_rescoring num_sleep sleep_overview sleep_period_time wake_time minutes_moving ...
    disp_date_sleep_start disp_rescored_sleep_start_pos disp_rescored_wake_up_pos disp_sleep_time date_sleep_start rescored_sleep_count ...
    s sec status err i j tspan xyz_hp sleep_count time_label disp_minutes_moving k n label lp hp ...
    rescored_sleep_start_pos rescored_wake_up_pos