...
Scoring_Threshold = 0.00025;    % 0.00025 for wrist, 
Wake_Threshold = 0.0005;        % 0.0005 for wrist, 0.00011 for ankle
...
%% Jean-Louis Scoring Algorithm
% Algorithm for Digital Integration as pre-processing

activity_count = digital_integration;
j = size(activity_count);
for i = 6:(j(2)-2)
    %for Digial Integration
    D(i) = 0.01 * (0.01*activity_count(i-4) + 0.015*activity_count(i-3) + 0.028 * activity_count(i-2) + 0.031 *activity_count(i-1) + 0.085*activity_count(i) + 0.015*activity_count(i+1) + 0.010*activity_count(i+2));
    %for Zero-crossings and time above threshold
    %D(i) = 0.01 * (0.1*activity_count(i-4) + 0.15*activity_count(i-3) + 0.28 * activity_count(i-2) + 0.31 *activity_count(i-1) + 0.85*activity_count(i) + 0.15*activity_count(i+1) + 0.10*activity_count(i+2));
end
...
axis normal
xlabel('Time')
title('MHSL Tracker, Sleep/Wake, Jean-Louis Scoring Algorithm')
ylim([-0.1 1.1])
hold on
...
clearvars sleep_period_min sleep_period sleep_start sleep_start_pos  wake_up wake_up_pos sleep_overwiev wake_time ...
    sleep_overview sleep_period_time wake_time minutes_moving ...
    disp_date_sleep_start  disp_sleep_time date_sleep_start rescored_sleep_count ...
    s sec status err i j tspan xyz_hp sleep_count time_label disp_minutes_moving k n label lp hp ...
     disp_rescored_sleep_start_pos disp_rescored_wake_up_pos num_sleep ...
     rescored_sleep_start_pos rescored_wake_up_pos sleep_rescoring
